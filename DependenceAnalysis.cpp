...
static cl::opt<unsigned> CacheLineSize(
	"cls",
	cl::desc("Specify cache line size in bytes"),
	cl::init(64),
	cl::value_desc("size"));

// Global variables
int loopNestCounter = 0;
...
// *************** LOOP COST IMPLEMENTATION **************

struct RefGroup {
  SmallVector<Instruction*> Members;
  bool IsTemporal;
  bool IsSpatial;
  Loop* ContainingLoop;  // Track which loop this group belongs to
};

bool checkTemporalReuse(Instruction &Ref1, Instruction &Ref2,
                const Loop &L, DependenceInfo &DI) {
  // Check dependence
  std::unique_ptr<Dependence> D = DI.depends(&Ref1, &Ref2, true);
  if (D == nullptr) {
    return false;
  }
  ...
  for (int Level = 1; Level <= Levels; ++Level) {
    ...
    if (Level != LoopDepth && !CI.isZero()) {
      return false;
    } else if (Level == LoopDepth && CI.getSExtValue() > 4) {
      return false;
    }
  }
...
bool checkSpatialReuse(raw_ostream &OS,
                       Instruction *Ref1,
                       Instruction *Ref2,
                       const Loop *L,
                       ScalarEvolution *SE) {
  Value* Ref1Ptr = getLoadStorePointerOperand(Ref1);
  Value* Ref2Ptr = getLoadStorePointerOperand(Ref2);

  const SCEV *SCEV1 = SE->getSCEV(Ref1Ptr);
  const SCEV *SCEV2 = SE->getSCEV(Ref2Ptr);

  // Check if references are for the same array
  if (Ref1Ptr == nullptr || Ref2Ptr == nullptr ||
      SE->getPointerBase(SCEV1) != SE->getPointerBase(SCEV2)) {
    return false;
      }

  // Extract GEP (GetElementPtr) instructions
  const GetElementPtrInst* GEP1 = dyn_cast<GetElementPtrInst>(Ref1Ptr);
  const GetElementPtrInst* GEP2 = dyn_cast<GetElementPtrInst>(Ref2Ptr);
  ...
  // Check all subscripts except first are identical
  unsigned subscriptsSize = GEP1->getNumIndices();
  for (unsigned i = 0; i < subscriptsSize - 1; ++i) {
    if (GEP1->getOperand(i+1) != GEP2->getOperand(i+1)) {
      return false;
    }
  }

  // Last first subscript difference
  Value* firstSub1 = GEP1->getOperand(subscriptsSize - 1);
  Value* firstSub2 = GEP2->getOperand(subscriptsSize - 1);

  ConstantInt* CI1 = dyn_cast<ConstantInt>(firstSub1);
  ConstantInt* CI2 = dyn_cast<ConstantInt>(firstSub2);

  // Compute constant distance
  if (CI1 && CI2) {
    int64_t distance = std::abs(CI1->getSExtValue() - CI2->getSExtValue());

    return distance < CacheLineSize;
  }

  return false;
}

void createRefGroups(raw_ostream &OS,
                     SmallVector<Instruction*> &Instructions,
                     SmallVector<RefGroup> &RefGroups,
                     Loop *L,
                     DependenceInfo *DI,
                     ScalarEvolution &SE) {

  for (size_t i = 0; i < Instructions.size(); ++i) {
    Instruction *Ref1 = Instructions[i];

    // Check if the instruction is already in a reference group
    bool inGroup = std::any_of(RefGroups.begin(), RefGroups.end(),
        [Ref1](const RefGroup& group) {
            return llvm::is_contained(group.Members, Ref1);
        });

    if (inGroup) continue;

    // Create a new reference group
    RefGroup NewRefGroup;
    NewRefGroup.Members.push_back(Ref1);
    NewRefGroup.IsTemporal = false;
    NewRefGroup.IsSpatial = false;

    // Find other instructions to group with
    for (size_t j = i + 1; j < Instructions.size(); ++j) {
      Instruction *Ref2 = Instructions[j];

      bool hasTemporal = checkTemporalReuse(*Ref1, *Ref2, *L, *DI);
      bool hasSpatial = checkSpatialReuse(OS, Ref1, Ref2, L, &SE);

      NewRefGroup.IsTemporal |= hasTemporal;
      NewRefGroup.IsSpatial |= hasSpatial;

      // Add to group if temporal or spatial reuse exists
      if (hasTemporal || hasSpatial) {
        NewRefGroup.Members.push_back(Ref2);
      }
    }

    RefGroups.push_back(NewRefGroup);
  }
}

Instruction* identifyLeader(SmallVector<Instruction*> &RefGroup,
                            DependenceInfo &DI) {

  Instruction* Leader = RefGroup[0];
  if (RefGroup.size() == 1)
    return Leader;

  for (Instruction *Ref1 : RefGroup) {
    for (Instruction *Ref2 : RefGroup) {
      if (Ref1 == Ref2) continue;

      // Get dependence
      std::unique_ptr<Dependence> D = DI.depends(Ref1, Ref2, true);

      // If direction is negative, skip
      if (D && !D->isDirectionNegative()) {
        Leader = Ref1;
        break;
      }
    }
...
// Helper function to determine the reuse type of a reference
ReuseType getReuseType(Instruction* Ref, Loop* L, ScalarEvolution& SE, unsigned CLS) {
    const SCEV* RefExpr = SE.getSCEV(getPointerOperand(Ref));
    if (const SCEVAddRecExpr* AR = dyn_cast<SCEVAddRecExpr>(RefExpr)) {
        if (AR->getLoop() == L) {
            const SCEV* Stride = AR->getStepRecurrence(SE);
            if (const SCEVConstant* C = dyn_cast<SCEVConstant>(Stride)) {
                int64_t StrideVal = C->getAPInt().getSExtValue();
                if (StrideVal == 0) {
                    return ReuseType::SelfTemporal; // Same location accessed repeatedly
                } else if (std::abs(StrideVal) < 4) {
                    return ReuseType::SelfSpatial; // Accessing nearby memory locations
                }
            }
        }
    }
    return ReuseType::NoSelfReuse; // Default case
}

int64_t calculateRefGroupCost(raw_ostream &OS,
                                Instruction *Leader,
                               Loop *L,
                               int64_t Trip,
                               ScalarEvolution &SE) {
    ...
    Value *Val = getLoadStorePointerOperand(Leader);
    if (L->isLoopInvariant(Val))
        return 1; // Cost for loop-invariant references

    ReuseType reuse = getReuseType(Leader, L, SE, CLS);

    switch (reuse) {
        case ReuseType::SelfTemporal:
            return 1; // Fixed cost for self-temporal reuse

        case ReuseType::SelfSpatial: {
            const SCEV* StrideExpr =
                cast<SCEVAddRecExpr>(
                    SE.getSCEV(getPointerOperand(Leader)))->getStepRecurrence(SE);
            int64_t Stride = cast<SCEVConstant>(StrideExpr)->getAPInt().getZExtValue();

            return ceil(Trip / (CacheLineSize / Stride));
        }

        case ReuseType::NoSelfReuse: {
            return Trip;
        }
    }
...
void computeLoopCost(raw_ostream &OS,
                   ScalarEvolution &SE, DependenceInfo &DI, LoopInfo &LI) {
  ...
  SmallVector<std::pair<const Loop *, int64_t>> TripCounts;
  for (const Loop *L : Loops) {
    int64_t TripCount = SE.getSmallConstantTripCount(L);
  ...
  // Get the basic block present in the loop body of the
  // inner most loop
  BasicBlock *BB = InnerMostLoop->getBlocks()[0];

  // Get all memory access instructions in the basic block
  SmallVector<Instruction*> MemoryAccessInstructions;
  for (Instruction &I : *BB) {
    ...
    MemoryAccessInstructions.push_back(&I);
  }

  OS << "\tNum. of Load/Store Instructions: " <<
      MemoryAccessInstructions.size() << "\n";

  // Consider each loop in the nest as inner loop
  for (Loop *L : Loops) {

    // Get loop control variable
    PHINode *InductionPHI = L->getInductionVariable(SE);
    if (!InductionPHI) {
      OS << "\tCould not find loop control variable.\n";
    }

    OS << "\tLoop: " << L->getName() << "\n";
    OS << "\tLoop control variable: " << *InductionPHI << "\n";

    // Compute the product of the trip counts of each other loop in the nest.
    int64_t TripCountsProduct = 1;
    ...
    // Initialize reference groups vector
    SmallVector<RefGroup> RefGroups;

    // Create reference groups for given loop
    createRefGroups(OS, MemoryAccessInstructions, RefGroups, L, &DI, SE);
    OS << "\tNum. of Ref Groups: " << RefGroups.size() << "\n";

    int64_t LoopCost = 0;

    // Find the leader and compute cost for each reference group
    for (size_t i = 0; i < RefGroups.size(); ++i) {
      OS << "\tRef Group " << i+1 << ":\n";

      // Print the ref group members
      OS << "\t\tMembers: \n";
      for (auto &I : RefGroups[i].Members) {
        OS << "\t\t\t" << *I << "\n";
      }

      // Find the leader of ref group
      Instruction* Leader = identifyLeader(RefGroups[i].Members, DI);
      OS << "\t\tLeader: " << *Leader << "\n";

      // Compute ref group cost
      int64_t TripCount = SE.getSmallConstantTripCount(L);
      int64_t RefGroupLeaderCost =
          calculateRefGroupCost(OS, Leader, L, TripCount, SE);

      int64_t RefGroupCost = RefGroupLeaderCost * TripCountsProduct;
      LoopCost += RefGroupCost;
      OS << "\t\tCost: " << RefGroupCost << "\n";
    }

    OS << "\tLoop Cost: " << LoopCost << "\n\n";
  }
}
...
DependenceAnalysisPrinterPass::run(Function &F, FunctionAnalysisManager &FAM) {
  ...
  computeLoopCost(OS, SE, DI, LI);

  return PreservedAnalyses::all();
}

// *************** END OF LOOP COST IMPLEMENTATION **************
